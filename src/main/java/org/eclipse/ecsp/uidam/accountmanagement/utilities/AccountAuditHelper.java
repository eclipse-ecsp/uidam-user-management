...
public class AccountAuditHelper {
    ...
    public void logAccountDeletedAudit(AccountEntity deletedAccount, BigInteger loggedInUserId,
        ...
        try {
            ...
            log.error("Failed to create audit log for account deletion: accountId={}",
                deletedAccount.getId(), e);
        }
    }
     ...
     * 
     * <p>Performance Note: This method uses minimal context (userId only) to avoid
     * additional database queries during audit logging. Audit logging happens synchronously
     * in the user management flow, so avoiding extra DB roundtrips is critical for performance.
     * The userId is sufficient for audit trail purposes and can be joined with user tables
     * for detailed reporting if needed.</p>
     *
     * @param loggedInUserId the ID of the logged-in user
     * @return UserActorContext with minimal user details
     */
    private UserActorContext buildAdminActorContext(BigInteger loggedInUserId) {
        ...
        // Use minimal context to avoid database query overhead
        // Username can be resolved via userId join in audit reports if needed
        return UserActorContext.builder()
    ...
    public String buildAccountStateJson(AccountEntity account) {
        try {
            ...
            log.error("Failed to build account state JSON", e);
            return null;
        }
    }
}
